...
if ($choice2 -eq "B") { Write-Host "`n `n `n Sorry, this option is not available right now. This is still a Work-In-Progress "
                        Break
...
if ($ErrorCatch)                       ###################################### ERROR HANDLING FOR INVALID UPN ######################################
    {                                                                                                                                                     #### ERROR HANDLING FOR INVALID UPN ###
        Write-Host " `n `n There was a problemn with getting the User details, please recheck the UPN, and make sure the user is not a guest account "          #### ERROR HANDLING FOR INVALID UPN ###
        Write-Host "  `nThe error is as below : `n `n $errorCatch "
        Break
    }
...
## Write-Host " Starting to check the resultant set of CA policies . . . . . . `n "                                                                                  ## Debug Info
...

{
   ...
   # Write-Host "`n Checking CA Policy `"$($x.displayName)`""                                  ############# Debug Info ######################
    $InFinal = $False                                                                          ############# We are using this temp parameter to decide in the end if a CA policy has to be in the resultant #########################
...
## // CheckExclusionUser 
        ...
        if ($userInforaw.id -in $Conditions.ExcludeUsers)                                                  ######## Checking User Exclusion ###############
         ...
         #   Write-Host "User Exclusion - Conditional Access policy `"$dp`" contains the User `"$userdp`" `( object ID - $userObjectID `) in Exclude Condition which the User is part of"              ######## Debug Info                                                    
            }
...
## // CheckExclusionGroup 
 ...
 $Groups | ForEach-Object {
       ...
       #             Write-Host "Group Exclusion- Conditional Access policy `"$($x.displayName)`" contains the group `"$($_.DisplayName)`" `( object ID - $($_.id) `) in Exclude Condition which the User is part of "        ### Debug Info
...
##Direct User Inclusion Check
  ...
  ##          Write-Host "Inclusion script running"                                       ### Debug Information ###
 ##           Write-Host " $InFinal "                                                     ### Debug Information ###
            }
...
if($endresult.Count -eq "0")                    ###################################### If NO Policies, then speak about it and Break ######################################
    ...
    Write-Host " `n There are no CA policies the user needs to be excluded from to bypass MFA from "               ########## Handling when there are no policies to exclude the user from  #############
    Break
    } 
else {
Write-Host " `n The policies considered for action : `n"

$endresult                                                                                                        ################# Resultant Set of CA Policies that we need to act on ###################
...
Foreach ($i in $capmemory)
    ...
    if ($req.StatusCode -eq 204)                                                                                                                                             ################# Success Status Code #################
        {

            if ($req2.conditions.users.excludeUsers -contains "$userObjectId") { Write-Host " `n Success making an exclusion with the policy $($req2.DisplayName) `n " }        ######## Checking if user is in exclusion ##############
            else { Write-Host " `n Exclusion does not seem to have been added to the policy $($req2.DisplayName) `n "}
        }
    else 
        { 
            Write-Host " `n Something went wrong in modifying the policy for Exclusion process to using Graph for policy $($i.DisplayName) `n "                                                                                        ###### Failure modifying policy through Graph ##########
        }   
    ...
    if ($req.StatusCode -eq 204)                                                                                                                                             ################# Success Status Code #################
        {
            ...
            if ($req2.conditions.users.excludeUsers -notcontains "$userObjectId") { Write-Host " `n Success reversing exclusion from policy $($req2.DisplayName) `n " }     ########### Checking if user is in exclusion ##############
            else { Write-Host " `n Reversing User Exclusion failed for policy $($req2.DisplayName) `n "}
        }
    else 
        { 
            Write-Host " `n Something went wrong in modifying the policy for CAP MFA ByPass reversal using Graph for policy $($i.DisplayName) `n "                         ######### Failure modifying policy through Graph ############
        }   
...
Switch ( $Choice2 )           #################################### Based on All Apps Condition, choose A or B ###########################
{
    A
        {
            ...
            Write-Host " `n We will now attempt to modify the policies to Exclude the User from the CA policies mentioned `n "
            Pause
            ...
            Write-Host " `n We will begin to check Sign in events to detect the next sign in for the user, post which we would go ahead to reverse the modifications done"
            Pause
            ...
            $SignInLogsURL = "https://graph.microsoft.com/beta/auditlogs/signins?$filter=startswith(UserPrincipalName," + "'" + $User + "'" + ")&" + "`$top=1" #| Write-Host
            ...
            While ($Finished -eq 0)                                                                                                                                        ###################### Waiting for success Sign in ###################
                {
                    ...
                    if ($t -gt $PolicyExcludedTime)                                                                                                                 ############## CHECK EVENT TIME ################
                            ...
                            if ($SignInEvent.status.errorCode -eq 0)                                                                                                ########## STATUS CODE FOR SUCCESS #############
                                    {
                                       ...
                                       Write-Host " `n We detected a successful Sign-in for the user! "
                                       $finished = 1
                                    }
                        }
                    else
                        {
                            ...
                            Write-Host " `n Waiting for Sign in events to generate the latest success event . . . . "
           ...
           Write-Host " `n We will now attempt to modify the policies to Exclude the User from the CA policies mentioned `n "
               ...
               CapStart                                                                                                                                                        ###################### CAP Start Function to Put User to Exclusion ###############
                ...
                Write-Host " `n We will now begin the timer ( $timer seconds ) post which the user will be removed from Exclusion `n"
               ...
               While ( $time -gt 0 )
               {
                    Sleep 1
                    $time--
                    Write-Host(" $time seconds remaining ")
               }


              ## Start-Sleep -Seconds $timer

               Write-Host " `n $timer of seconds has passed. We will reverse the Exclusion now "
           ...
           Write-Host " Default time is set to 10 minutes (600s) "
    ...
    B

      {
        ...
        Write-Host " `n `n `n Sorry, this is not available right now. This is still a Work-In-Progress."
